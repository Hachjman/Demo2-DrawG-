...
module.exports = function(gameNamespace) {
  // Helper: start progressive hints for the current room
  function startProgressiveHints(room) {
    // Clear any previous interval
    room.clearHintInterval?.();
    const intervalSec = typeof config.HINT_INTERVAL_SEC === 'number' && config.HINT_INTERVAL_SEC > 0
      ? config.HINT_INTERVAL_SEC
      : 15;
    if (!room.currentWord) return;
    const word = room.currentWord;

    // Initialize revealed indices (none at start)
    room.hintRevealedIndices = [];

    const emitHint = () => {
      if (!room.currentWord) return;
      const arr = word.split('');
      const hintArr = arr.map((ch, idx) => (room.hintRevealedIndices.includes(idx) ? ch : '_'));
      const hint = hintArr.join('');
      // Emit to guessers only (exclude current drawer)
      const drawerId = room.currentDrawerId;
      for (const p of room.players) {
        if (p.id !== drawerId) {
          gameNamespace.to(p.id).emit('word-hint', { hint, length: word.length });
        }
      }
    };

    // First emit is already sent when word is selected (all underscores), schedule reveals
    room.hintInterval = setInterval(() => {
      if (!room.currentWord) return;
      // Find unrevealed indices
      const unrevealed = [];
      for (let i = 0; i < word.length; i++) {
        if (!room.hintRevealedIndices.includes(i) && /\S/.test(word[i])) {
          unrevealed.push(i);
        }
      }
      ...
      room.hintRevealedIndices.push(idx);
      emitHint();
    }, intervalSec * 1000);
  }
        ...
        if (message.toLowerCase() === room.currentWord.toLowerCase()) {
            ...
            if (!room.correctGuessers.has(socket.id) && socket.id !== room.currentDrawerId) {
                ...
                socket.emit('chat-message', {
                    ...
                    playerName: 'Hệ thống',
                    playerAvatar: '🤖',
                    message: 'Bạn đã đoán đúng rồi! Hãy đợi vòng tiếp theo.'
                });
                return;
            }
        } else {
        ...
        if (drawerSocket) {
            ...
            room.wordSelectTimer = setTimeout(() => {
                if (!room.currentWord) {
                    ...
                    // Notify drawer of chosen word
                    const drawerSock = gameNamespace.sockets.get(drawerId);
                    if (drawerSock) drawerSock.emit('word-selected', { word: autoWord });
                    // Send initial hint to guessers only
                    const hint = autoWord.replace(/\S/g, '_');
                    for (const p of room.players) {
                      if (p.id !== drawerId) {
                        gameNamespace.to(p.id).emit('word-hint', { hint, length: autoWord.length });
                      }
                    }
                    room.startTimer(() => handleRoundEnd(room));
                    startProgressiveHints(room);
