...
(() => {
  ...
  let lastWordOptions = [];
  function clearWordSelectTimer() {
    if (wordSelectInterval) {
      clearInterval(wordSelectInterval);
      wordSelectInterval = null;
    }
    const container = wordModal ? wordModal.querySelector('.modal-content') : null;
    const countdownEl = container ? container.querySelector('#wordCountdown') : null;
  ...
  socket.on('next-round', (data) => {
    ...
    // Ensure any previous word selection timers are cleared
  ...
  function showWordSelection(words) {
    // Save options for possible auto-pick
    lastWordOptions = Array.isArray(words) ? words.slice(0, 3) : [];
    // Reset UI and any previous timer
    clearWordSelectTimer();
    ...
    // Insert countdown element under title
    const container = wordModal.querySelector('.modal-content');
    let countdownEl = container.querySelector('#wordCountdown');
    if (!countdownEl) {
      countdownEl = document.createElement('div');
      countdownEl.id = 'wordCountdown';
      countdownEl.style.margin = '8px 0 0';
      countdownEl.style.color = '#94a3b8';
      countdownEl.style.fontWeight = '700';
      countdownEl.style.fontSize = '14px';
      container.insertBefore(countdownEl, wordChoices);
    }

    let remaining = 8;
    countdownEl.textContent = `Auto-select in ${remaining}s`;
    wordModal.classList.remove('hidden');

    wordSelectInterval = setInterval(() => {
      remaining -= 1;
      if (remaining <= 0) {
        // Auto pick randomly
        clearWordSelectTimer();
        const pool = lastWordOptions.length ? lastWordOptions : words;
        const randomWord = pool[Math.floor(Math.random() * pool.length)] || words[0];
